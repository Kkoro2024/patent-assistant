...
import { Scale, Lightbulb, Search, FileText, ShieldAlert, TrendingUp, ArrowRight, Zap, Globe, Lock } from "lucide-react";
...
const FEATURES = [
  {
    ...
    description: "Compare two technology fields head-to-head by patent volume, growth rate, and top companies.",
  },
  {
    icon: Lightbulb,
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <span style={{ fontWeight: "bold", letterSpacing: "0.08em", fontSize: "15px" }}>PATENT LAW AI</span>
          </div>
          <motion.button
        ...
        <section style={{ textAlign: "center", padding: "120px 24px 80px" }}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }}>
            ...
            <h1 style={{ fontSize: "clamp(42px, 7vw, 88px)", fontWeight: "bold", lineHeight: 1.05, marginBottom: "24px", letterSpacing: "-0.02em" }}>
              Patent Law,<br />
              <span style={{ background: "linear-gradient(135deg, #c9a84c, #e8c97a, #a07830)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                Demystified.
              </span>
            </h1>
            <p style={{ fontSize: "clamp(16px, 2vw, 20px)", color: "rgba(255,255,255,0.5)", maxWidth: "560px", margin: "0 auto 48px", lineHeight: 1.7, fontFamily: "system-ui" }}>
              An AI patent attorney in your pocket. Search real patents, evaluate your ideas, draft claims, and check for infringement — instantly.
            </p>
            <motion.button
        ...
        >
          {STATS.map((stat, i) => (
            <div key={stat.label} style={{
              ...
              <div style={{ fontSize: "32px", fontWeight: "bold", color: "#c9a84c", marginBottom: "4px" }}>{stat.value}</div>
              <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", letterSpacing: "0.1em", fontFamily: "system-ui" }}>{stat.label.toUpperCase()}</div>
            </div>
          ))}
        </motion.section>
        ...
        <section style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 24px 100px" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ textAlign: "center", marginBottom: "64px" }}>
            <h2 style={{ fontSize: "clamp(28px, 4vw, 44px)", fontWeight: "bold", marginBottom: "16px" }}>Six tools. One platform.</h2>
            <p style={{ color: "rgba(255,255,255,0.4)", fontFamily: "system-ui", fontSize: "16px" }}>Everything a founder, inventor, or law student needs to navigate IP.</p>
          </motion.div>
            ...
            {FEATURES.map((f, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{
                    border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", padding: "28px",
                    background: "rgba(255,255,255,0.02)", transition: "border-color 0.2s, background 0.2s", cursor: "default",
                  }}
                >
                  <div style={{
                  ...
                  <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.45)", lineHeight: 1.6, fontFamily: "system-ui" }}>{f.description}</p>
                </motion.div>
              );
            })}
          </div>
          ...
          {[
            ...
            return (
              <motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} style={{ textAlign: "center" }}>
                <div style={{ width: "48px", height: "48px", borderRadius: "50%", margin: "0 auto 16px", background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Icon size={20} color="#c9a84c" />
                </div>
                <h3 style={{ fontSize: "17px", fontWeight: "bold", marginBottom: "10px" }}>{item.title}</h3>
                <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", lineHeight: 1.7, fontFamily: "system-ui" }}>{item.body}</p>
              </motion.div>
            );
          })}
        </section>
        ...
        >
          <h2 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: "bold", marginBottom: "16px" }}>Ready to research?</h2>
          <p style={{ color: "rgba(255,255,255,0.4)", fontFamily: "system-ui", marginBottom: "40px", fontSize: "16px" }}>Free to use. No signup. Just ask.</p>
          <motion.button
            whileHover={{ scale: 1.04, boxShadow: "0 0 40px rgba(201,168,76,0.4)" }} whileTap={{ scale: 0.97 }}
            onClick={() => setLaunched(true)}
            style={{
              background: "linear-gradient(135deg, #c9a84c, #a07830)", border: "none", borderRadius: "14px",
              padding: "18px 56px", color: "black", fontWeight: "bold", fontSize: "16px",
              cursor: "pointer", letterSpacing: "0.04em", fontFamily: "system-ui",
        ...
        <footer style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "24px 40px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ width: "28px", height: "28px", borderRadius: "8px", background: "linear-gradient(135deg, #c9a84c, #a07830)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Scale size={12} color="black" />
            </div>
            <span style={{ fontSize: "13px", fontWeight: "bold", letterSpacing: "0.06em" }}>PATENT LAW AI</span>
          </div>
          <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.2)", fontFamily: "system-ui" }}>For informational purposes only · Not legal advice</p>
        </footer>
      </div>
    </div>
  );
